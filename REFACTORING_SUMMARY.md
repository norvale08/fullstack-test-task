...
#### 3. Repository Pattern (`src/repositories.py`)
...
#### 4. Service Layer Refactoring (`src/service.py`)
...
#### 5. File Storage Service (`src/services/file_storage.py`)
- **Before**: File operations scattered across `service.py` and `tasks.py`
- **After**: Dedicated file storage service
...
#### 6. File Scanner Service (`src/services/file_scanner.py`)
- **Before**: Threat detection and metadata extraction logic embedded in `tasks.py`
- **After**: Dedicated file scanner service
...
#### 9. API Layer Cleanup (`src/app.py`)
...
**Database Connection Pooling**: The main optimization was implementing proper connection pooling with tuned parameters. This eliminates the overhead of creating new connections for each request and improves performance under load.
...
#### 5. Main Page Refactoring (`src/app/page.tsx`)
...
- **Performance**: Optimized connection pooling reduces database overhead
...
The refactored application runs the same way as before:
